...
private:
   ...
   * Add any helper methods you may need
   */
  int Min(int a, int b, int c) const { return std::min(std::min(a, b), c); }

  int PathStartIdx(const int arr[], int size) const {
    int idx = 0, min_val = arr[0];
    for (int i = 0; i < size; ++i)
      if (arr[i] < min_val) idx = i, min_val = arr[i];
    return idx;
  }
};
